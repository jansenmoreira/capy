...
int test_smap(void)
{
    ...
    for (int i = 0; i < 11; i++)
    {
        ...
        expect(strcmp(pair.key, tests[i].key) == 0);
        expect(pair.value.x == tests[i].value.x);
    }

    pair = capy_smap_get_point(&smap1, tests[11].key);
    expect(pair.key == NULL);
...
int test_sset(void)
{
    ...
    for (int i = 0; i < 11; i++)
    {
        ...
        expect(strcmp(value, tests[i].key) == 0);
    }

    expect(capy_sset_get(&sset1, tests[11].key) == NULL);
    ...
    expect(capy_sset_get(&sset1, tests[10].key) == NULL);
...
int test_symbols(void)
{
    ...
    expect(str_a1 == str_a2);
    expect(str_a1 == str_a3);
    expect(str_a1 == str_a4);
    ...
    expect(str_b1 == str_b2);
    expect(str_b1 == str_b3);
